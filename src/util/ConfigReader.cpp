...
ConfigReader::ConfigReader(const char* filename, string delimiter,string comment)
...
std::istream& operator >> (std::istream& is, ConfigReader& cf){
    ...
    while (is || nextline.length() > 0)
    {
        ...
        if (delimPos < string::npos)
        {
            ...
            // StopThis at blank line, next line with a key, end of stream,
...
const map<std::string, std::string> &ConfigReader::GetContents() const {
    return m_Contents;
}
