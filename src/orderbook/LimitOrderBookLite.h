...
namespace marketdata
{
    ...
    private:
        ...
        // 共享价格档位池，PriceLevelBook 统一从这里申请/释放价格档位
    ...
    public:
        explicit LimitOrderBookLite(const std::string date,
            ...
            _date(date), _code(code), level_pool(levelPoolPtr),
            ...
            pending_sell_book(min_price, max_price, 10000, level_pool.get())
        {
            if (!level_pool)
            {
                LOG_ERROR(app_log::logger(), "PriceLevelPool is null, date:{} code:{}", date, code);
                STDTHROW(STD_ERROR_CODE, "PriceLevelPool is null", "PriceLevelPool is null");
            }
